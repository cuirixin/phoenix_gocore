...
func (menu *Menu) SetMenu(buttons []*Button) error {
	...
	response, err := utils.PostJSON(uri, reqMenu)
	if err != nil {
		return err
	}
	var commError utils.CommonError
...
func (menu *Menu) GetMenu() (resMenu ResMenu, err error) {
	...
	var response []byte
	response, err = utils.HTTPGet(uri)
	if err != nil {
		return
	}
...
func (menu *Menu) DeleteConditional(menuID int64) error {
	...
	response, err := utils.PostJSON(uri, reqDeleteConditional)
	if err != nil {
		return err
	}
	var commError utils.CommonError
...
func (menu *Menu) MenuTryMatch(userID string) (buttons []Button, err error) {
	...
	var response []byte
	response, err = utils.PostJSON(uri, reqMenuTryMatch)
	if err != nil {
		return
	}
