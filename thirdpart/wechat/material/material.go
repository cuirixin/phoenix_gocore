...
func (material *Material) AddNews(articles []*Article) (mediaID string, err error) {
	...
	responseBytes, err := utils.PostJSON(uri, req)
	var res resArticles
...
func (material *Material) AddMaterial(mediaType MediaType, filename string) (mediaID string, url string, err error) {
	...
	var response []byte
	response, err = utils.PostFile("media", filename, uri)
	if err != nil {
		return
	}
...
func (material *Material) AddVideo(filename, title, introduction string) (mediaID string, url string, err error) {
	...
	fields := []utils.MultipartFormField{
		{
			IsFile:    true,
			Fieldname: "video",
			Filename:  filename,
	...
	var response []byte
	response, err = utils.PostMultipartForm(fields, uri)
	if err != nil {
		return
	}
...
func (material *Material) DeleteMaterial(mediaID string) error {
	...
	response, err := utils.PostJSON(uri, reqDeleteMaterial{mediaID})
	if err != nil {
		return err
	}
	var resDeleteMaterial utils.CommonError
